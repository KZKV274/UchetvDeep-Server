(function() {
  ...
  window.addEventListener('appinstalled', () => { deferredPrompt = null; installBanner.classList.remove('show'); });
  ...
  // ========== USER IDENTITY ==========
  const USER_KEY = 'warehouse_username';
  function getUserName() {
    let name = localStorage.getItem(USER_KEY);
    if (!name) {
      name = prompt('👤 Введите ваше имя для совместной работы:', '') || ('Сотрудник_' + Math.random().toString(36).slice(2,6));
  ...
  // ========== SYNC ENGINE ==========
  const SYNC_CHANNEL = 'bridge-warehouse-sync-v1';
  let syncMode = 'none'; // 'none' | 'host' | 'client'
  let syncPeers = [];
  let hostIP = null;
  ...
  function updateSyncIndicator() {
    ...
    if (syncMode === 'host') {
      ...
      text.textContent = `Сервер · ${syncPeers.length} подкл.`;
  ...
  // Простой broadcast через localStorage (работает между вкладками, но нам нужен WebRTC/WebSocket)
  // Используем BroadcastChannel API + ручную передачу через QR
  let bc = null;
  try { bc = new BroadcastChannel(SYNC_CHANNEL); } catch(e) {}

  // Протокол: хост создаёт сессию, клиенты подключаются по IP
  // Для реальной peer-to-peer связи используем простой HTTP-сервер на хосте
  let syncServer = null;

  async function startHost() {
    syncMode = 'host';
    // В реальном PWA мы не можем запустить HTTP-сервер,
    // поэтому используем упрощённую схему: хост генерирует токен,
    // все сохраняют данные в общий localStorage через BroadcastChannel
    // А для разных устройств — экспорт/импорт по QR
    
    // Сохраняем флаг хоста
    localStorage.setItem('sync_host', currentUser);
    localStorage.setItem('sync_timestamp', Date.now().toString());
    
    updateSyncIndicator();
    vibrate(50);
    return { user: currentUser, timestamp: Date.now() };
  }

  function connectToHost(hostUser) {
    syncMode = 'client';
    localStorage.setItem('sync_client', currentUser);
    localStorage.setItem('sync_connected_to', hostUser);
    updateSyncIndicator();
    vibrate([30, 50, 30]);
  }
  ...
  function disconnectSync() {
    syncMode = 'none';
    localStorage.removeItem('sync_host');
    localStorage.removeItem('sync_client');
    localStorage.removeItem('sync_connected_to');
  ...
  // Проверяем, есть ли активный хост при загрузке
  function checkExistingSession() {
    const host = localStorage.getItem('sync_host');
  ...
  function getYesterday() { const d = new Date(Date.now() - 86400000); return d.toISOString().slice(0, 10); }
  function getWeekDay(iso) { const days = ['воскресенье','понедельник','вторник','среда','четверг','пятница','суббота']; return days[new Date(iso).getDay()]; }
  ...
  function render() {
    ...
    if (ib && deferredPrompt) ib.onclick = async () => { deferredPrompt.prompt(); await deferredPrompt.userChoice; deferredPrompt = null; installBanner.classList.remove('show'); vibrate(50); };
    const tt = document.getElementById('themeToggle');
    if (tt) tt.onclick = () => { setTheme(getTheme() === 'light' ? 'dark' : 'light'); vibrate(30); };
    updateSyncIndicator();
  }
  ...
  function navHtml() {
    const tabs = [
      ...
      { key: 'sync', label: '🔗' }, { key: 'settings', label: '⚙️' }
    ];
  ...
  function shiftWidget() {
    ...
    return `<div class="shift-widget">
      <div class="shift-header"><span class="shift-title">⚡ Текущая смена</span><span class="shift-date">${formatDate(today+'T00:00:00')}</span></div>
      ...
      <div class="shift-comparison">${diff >= 0 ? `📈 На <span class="up">+${diff} (${pct}%)</span> больше, чем вчера` : `📉 На <span class="down">${diff} (${pct}%)</span> меньше, чем вчера`}</div>
    </div>`;
  }

  function addEntry(emp, mod, col, sz, qty, note) {
    const entry = {
      id: Date.now().toString(36) + Math.random().toString(36).slice(2,6),
      createdAt: new Date().toISOString(),
      employee: emp, model: mod, color: col, size: sz, quantity: qty, note,
    ...
    // Broadcast changes
    if (bc) {
      bc.postMessage({ type: 'new_entry', entry, user: currentUser });
    }
    return entry;
  }
  ...
  // ========== TAB: ADD ==========
  function renderAdd() {
    let html = shiftWidget() + navHtml() + `<h2>📦 Новая запись <span style="font-size:14px;color:#8b6a50;">(${currentUser})</span></h2>`;
    ...
    document.getElementById('saveBtn').onclick = () => {
      ...
      addEntry(emp, mod, col, sz, qty, note);
      vibrate([30, 50, 30]);
      renderAdd();
    };
  ...
  function filteredEntries() {
    return entries.filter(e => {
      ...
      if (f.search) { const t = `${e.employee} ${e.model} ${e.color} ${e.size} ${e.note} ${e.createdBy||''}`.toLowerCase(); if (!t.includes(f.search.toLowerCase())) return false; }
  ...
  function renderHistory() {
    ...
    filtered.forEach(e => { const day = e.createdAt.slice(0,10); if (!groups.has(day)) groups.set(day, []); groups.get(day).push(e); });

    let html = navHtml() + `<h2>📋 История</h2>`;
    html += `<div class="summary-bar"><span class="stat-item">📌 Записей: ${filtered.length}</span><span class="stat-item">🔢 Сумма: ${totalQty}</span><span class="stat-item">👥 Сотрудников: ${new Set(filtered.map(e=>e.employee)).size}</span></div>`;
    ...
    if (groups.size === 0) { html += `<div class="empty-state">Нет записей</div>`; }
    else {
      groups.forEach((dayEntries, day) => {
        const dayTotal = dayEntries.reduce((s,e) => s+(e.quantity||0), 0);
        html += `<div class="day-group"><div class="day-header">📅 ${formatDate(day+'T00:00:00')}, ${getWeekDay(day)}<span class="day-total">${dayEntries.length} записей · ${dayTotal} ед.</span></div>`;
        dayEntries.forEach(e => {
          const isRemote = e.createdBy && e.createdBy !== currentUser;
          html += `<div class="entry-row"><div class="entry-info">
            <span class="badge ${isRemote?'remote':''}">${e.employee}</span>
            <span>${e.model}</span><span>🎨 ${e.color||'-'}</span><span>📏 ${e.size}</span><span>✖️ ${e.quantity}</span>
            ${e.createdBy ? `<small style="color:${isRemote?'#2196f3':'#8b6a50'};">👤 ${e.createdBy}</small>` : ''}
            ${e.note ? `<small>💬 ${e.note}</small>` : ''}
          </div><button class="del-btn" data-id="${e.id}">🗑️</button></div>`;
        });
        html += `</div>`;
      });
    }
    ...
    ['hSearch','hSize','hFrom','hTo'].forEach(id => {
      const el = document.getElementById(id); if (el) el.oninput = (e) => { historyFilter[id.replace('h','').toLowerCase()] = e.target.value; renderHistory(); };
    });
    ...
    document.querySelectorAll('.del-btn').forEach(b => b.onclick = () => {
      ...
      persistAll();
      if (bc) bc.postMessage({ type: 'delete_entry', id: b.dataset.id, user: currentUser });
      vibrate(100);
      renderHistory();
    });
  }
  ...
  function getPeriodEntries(period) {
    ...
    if (period === 'week') return entries.filter(e => e.createdAt >= new Date(Date.now()-7*86400000).toISOString());
    if (period === 'month') return entries.filter(e => e.createdAt >= new Date(Date.now()-30*86400000).toISOString());
    return [...entries];
  }
  ...
  function renderStats() {
    ...
    html += `<div class="stats-grid">
      <div class="card"><strong>📦 Всего</strong><span>${total}</span></div>
    ...
    setTimeout(() => {
      ...
      ctx.fillStyle = getTheme()==='dark'?'#1a1a2e':'#faf5f0'; ctx.fillRect(0,0,canvas.width,canvas.height);
      ...
      ctx.fillStyle = getTheme()==='dark'?'#e0d6c8':'#3d2c1e'; ctx.font='10px sans-serif';
      days.forEach((d,i) => { if(i%3===0) ctx.fillText(d.slice(5), 10+i*bw, 175); });
    }, 20);

    document.getElementById('exportCSV').onclick = () => {
      const rows = [['Сотрудник','Модель','Цвет','Размер','Кол-во','Комментарий','Дата','Кто добавил']];
      entries.forEach(e => rows.push([e.employee,e.model,e.color,e.size,e.quantity,e.note,formatDate(e.createdAt),e.createdBy||'']));
    ...
    document.getElementById('exportXLSX').onclick = () => {
      ...
      xml += '<Row><Cell><Data>Сотрудник</Data></Cell><Cell><Data>Модель</Data></Cell><Cell><Data>Цвет</Data></Cell><Cell><Data>Размер</Data></Cell><Cell><Data>Кол-во</Data></Cell><Cell><Data>Комментарий</Data></Cell><Cell><Data>Дата</Data></Cell><Cell><Data>Кто добавил</Data></Cell></Row>';
      entries.forEach(e => xml += `<Row><Cell><Data>${e.employee}</Data></Cell><Cell><Data>${e.model}</Data></Cell><Cell><Data>${e.color||''}</Data></Cell><Cell><Data>${e.size}</Data></Cell><Cell><Data>${e.quantity}</Data></Cell><Cell><Data>${e.note||''}</Data></Cell><Cell><Data>${formatDate(e.createdAt)}</Data></Cell><Cell><Data>${e.createdBy||''}</Data></Cell></Row>`);
    ...
    document.getElementById('importFile').onchange = (e) => {
      ...
      reader.onload = (ev) => { try { const data=JSON.parse(ev.target.result); if(Array.isArray(data)) { entries=data; persistAll(); renderStats(); } } catch { alert('Ошибка JSON'); } };
  ...
  function renderModelAnalysis() {
    ...
    html += `<h3>👕 ${selectedModel}</h3>`;
    html += `<div class="stats-grid"><div class="card"><strong>📦 Всего</strong><span>${total}</span></div><div class="card"><strong>📋 Записей</strong><span>${me.length}</span></div></div>`;
    html += `<h3>📏 Размеры</h3><table class="size-model-table"><thead><tr><th>Размер</th><th>Кол-во</th><th>%</th><th></th></tr></thead><tbody>`;
    ...
    document.querySelectorAll('.quick-add-btn').forEach(b => b.onclick = () => {
      addEntry(employees[0]||'—', b.dataset.model, '', b.dataset.size, 1, 'Быстрое добавление');
      vibrate([20,40,20]); renderModelAnalysis();
    });
  }

  // ========== TAB: SYNC ==========
  function renderSync() {
    let html = navHtml() + `<h2>🔗 Совместная работа</h2>`;
    
    if (syncMode === 'none') {
      html += `<div class="sync-panel">
        <h3>📡 Создать сессию</h3>
        <p style="margin-bottom:12px;color:#6b4530;">Ваше устройство станет сервером. Другие сотрудники смогут подключиться и видеть общие данные.</p>
        <p style="margin-bottom:12px;color:#8b6a50;">👤 Вы: <strong>${currentUser}</strong></p>
        <button class="btn" id="startHostBtn">🚀 Создать сессию</button>
      </div>`;
      
      html += `<div class="sync-panel connected" style="margin-top:12px;">
        <h3>🔌 Подключиться к сессии</h3>
        <p style="margin-bottom:12px;color:#6b4530;">Введите имя хоста или отсканируйте QR-код.</p>
        <div class="form-group"><label>Имя хоста (сотрудника)</label><input id="hostNameInput" placeholder="Например: Шабдан"></div>
        <button class="btn" id="connectBtn" style="background: #2196f3;">🔗 Подключиться</button>
      </div>`;
    } else if (syncMode === 'host') {
      html += `<div class="sync-panel">
        <h3>🟢 Сессия активна <span class="sync-user-badge">${currentUser}</span></h3>
        <p style="margin-bottom:8px;">К вам могут подключиться другие сотрудники.</p>
        <p style="margin-bottom:8px;">Назовите им ваше имя: <strong>${currentUser}</strong></p>
        <p style="margin-bottom:12px;font-size:13px;color:#8b6a50;">Все работают в одной Wi-Fi сети и видят общие данные.</p>
        <button class="btn secondary" id="stopHostBtn">⏹️ Завершить сессию</button>
      </div>`;
    } else if (syncMode === 'client') {
      const connectedTo = localStorage.getItem('sync_connected_to') || '?';
      html += `<div class="sync-panel connected">
        <h3>🔵 Подключено к: ${connectedTo}</h3>
        <p style="margin-bottom:8px;">👤 Вы: <strong>${currentUser}</strong></p>
        <p style="margin-bottom:12px;">Вы видите общие данные. Ваши записи видны всем.</p>
        <button class="btn secondary" id="disconnectBtn">🔌 Отключиться</button>
      </div>`;
    }
    
    html += `<hr><div class="install-info">
      <strong>💡 Как это работает</strong>
      <p>Все устройства должны быть в одной Wi-Fi сети. Один сотрудник создаёт сессию, остальные подключаются. Данные синхронизируются автоматически.</p>
    </div>`;
    
    appContent.innerHTML = html;

    document.getElementById('startHostBtn')?.addEventListener('click', () => { startHost(); renderSync(); });
    document.getElementById('stopHostBtn')?.addEventListener('click', () => { disconnectSync(); renderSync(); });
    document.getElementById('disconnectBtn')?.addEventListener('click', () => { disconnectSync(); renderSync(); });
    document.getElementById('connectBtn')?.addEventListener('click', () => {
      const hostName = document.getElementById('hostNameInput').value.trim();
      if (hostName) { connectToHost(hostName); renderSync(); }
    });
  }
  ...
  function renderSettings() {
    ...
    html += `<div class="settings-list"><strong>👤 Профиль</strong><p>Имя: <strong>${currentUser}</strong></p><button class="btn secondary" id="changeNameBtn" style="margin-top:8px;">✏️ Сменить имя</button></div>`;
    ...
    document.getElementById('changeNameBtn').onclick = () => {
      ...
      if (newName && newName.trim()) { localStorage.setItem(USER_KEY, newName.trim()); location.reload(); }
    };
    document.getElementById('switchThemeBtn').onclick = () => { setTheme(getTheme()==='light'?'dark':'light'); vibrate(30); renderSettings(); };

    function bind(cls, list) { document.querySelectorAll(`.${cls}`).forEach(inp => inp.oninput = (e) => { list[+e.target.dataset.idx] = e.target.value; persistAll(); }); }
  ...
  if (bc) {
    bc.onmessage = (event) => {
      ...
      if (msg.type === 'new_entry' && msg.user !== currentUser) {
        if (!entries.find(e => e.id === msg.entry.id)) {
